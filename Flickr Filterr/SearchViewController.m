...
- (void) searchButtonPressed
{
  ...
  //  @try
   // {
//        NSData * flickrData = [NSData dataWithContentsOfURL: request];
//        NSError * flickrError = nil;
        self.connection = [self makeConnection];
        [self.connection start];
   //     NSDictionary * flickrDict = self.connection;
        
//    }
//    @catch (NSException * exception)
//    {
//        UIAlertView * errorMessage = [[UIAlertView alloc] initWithTitle: @"Error Occurred" message: [@"Internet is probably not working. " stringByAppendingString: exception.reason] delegate:nil cancelButtonTitle:@"Continue" otherButtonTitles: nil];
//        [errorMessage show];
//        return;
//    }
//    self.connection = [self makeConnection];
...
-(FSNConnection *) makeConnection{
    ...
    return [FSNConnection withUrl:request method:FSNRequestMethodGET headers:nil parameters:nil parseBlock:^id(FSNConnection *c, NSError **error){
     ...
     //   NSLog(@"complete: %@\n\nerror: %@\n\n", c, c.error);
        NSLog(@"FINISHED DDDDDDDD DDDDDD MAKING CONNECTION YAYAY");

//        NSDictionary * flickrDict = [NSJSONSerialization JSONObjectWithData: flickrData options: 0 error: &flickrError];
        NSError *error = nil;
...
- (void) imageViewPressed: (UITapGestureRecognizer *) sender
{
    ...
    self.connection = [self filterHelper];
    [self.connection start];
